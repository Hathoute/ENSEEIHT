...
%
    ...
    lignes = size(donnees);
    lignes = lignes(1);
    J_res = zeros(lignes, length(beta));
...
%
    ...
    res = residu(beta);
    J_res = J_residu(beta);
    H_f = zeros(length(beta));

    H_f(1,1) = sum(exp(-2*beta(2)*donnees(:,1)));
    H_f(1,2) = sum(donnees(:,2).*donnees(:,1).*exp(-beta(2)*donnees(:,1)) ...
                    - 2*beta(1)*donnees(:,1).*exp(-2*beta(2)*donnees(:,1)));
    H_f(2,1) = H_f(1,2);
    H_f(2,2) = sum(- donnees(:,2).*(donnees(:,1).^2)*beta(1).*exp(-beta(2)*donnees(:,1)) ...
                    + 2*beta(1)^2*(donnees(:,1).^2).*exp(-2*beta(2)*donnees(:,1)));
