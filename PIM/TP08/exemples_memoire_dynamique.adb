...
procedure Exemples_Memoire_Dynamique is
	...
	procedure Illustrer_Memoire_Dynamique_Sans_Free is
		Ptr1 : T_Pointeur_Integer;
	begin
		Ptr1 := new Integer; 			-- Le problème venait de cette partie.
		Ptr1.all := 5;
		Put_Line ("Ptr1.all = " & Integer'Image (Ptr1.all));
		Free(Ptr1);
	...
	begin
		...
		-- La valeur de Ptr2.all sera 0, car la place memoire où pointe ce pointeur
		--	est déja libérée par le pointeur Ptr1.
		-- 1 Put_Line ("Ptr2.all = " & Integer'Image (Ptr2.all));

		-- 2 Ptr2.all := 7;
		-- 3 pragma Assert (Ptr2.all = 7);

		-- XXX A-t-on le droit de manipuler Ptr2.all ?
		--	De point de vue possibilité, oui on a le droit. Mais de point de vue
		--	pratique, non car une nouvelle allocation va interferer avec cette valeur.
		-- XXX Que se passe-t-il si on exécute le programme avec valkyrie ?
		--	On obtient des invalid reads/writes, car on manipule des bloques qui devraient
		--	être libérées.
	...
	begin
		...
		--	Oui, on doit utiliser Detruire dessus car liberer T_Pile revient à liberer
		--	les valeurs des pointeurs eux même, non pas les données où les pointeurs pointent. 
